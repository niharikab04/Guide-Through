...
async function getRandomPostDescription(userName) {
    try {
        const userData = await Interest.findOne({username:userName});
        const interestsArr = userData.interests;
        console.log("interests ",interestsArr);
        const posts = await Post.find({ category: { $in: interestsArr},section: { $in: ["No-gos", 'Advice'] } });
...
module.exports.get_tipnotification = async (req, res) => {
    ...
    if (userData) {
        ...
        const randomPostDescription = await getRandomPostDescription(userName);
...
module.exports.post_interests = async (req,res) =>{
    try{
        const { username, selectedInterests } = req.body;
        const userDoc = await Interest.create({
            username: username,
            interests: selectedInterests
        })
        const userDoc2 = await User.findOne({username});
        const id = userDoc2._id;
        await tipNotification.create({ userinfo: id });
        console.log(userDoc);
...
module.exports.get_interests = async (req,res) =>{
    const {username} = req.params;
    try {
        const userDoc = await Interest.findOne({username});
        if (!userDoc) {
            return res.status(404).json({ message: 'User not found' });
        }
        else {
            console.log(userDoc.interests);
            return res.status(200).json({ interests: userDoc.interests });
        }
    }
    catch (err) {
        console.log(err)
...
module.exports.edit_interests = async (req,res) =>{
    try{
        const { username, selectedInterests } = req.body;
        const userDoc = await Interest.findOne({
            username: username,
        })
        userDoc.interests = selectedInterests;
        await userDoc.save();
        console.log(userDoc);
        res.status(200).json(userDoc);
